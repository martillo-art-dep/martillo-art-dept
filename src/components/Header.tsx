...
/* ─── Social icon hover: ONLY the path changes, bg stays ─── */
...
// ═══════════════════════════════════════════════════════════════════════
// Inline SVG Icons — allows hover to target ONLY the icon path
// ═══════════════════════════════════════════════════════════════════════
...
function VimeoIcon({ color }: { color: string }) {
  return (
    <svg width="26" height="22" viewBox="0 0 24 24" fill="none">
      <path className="icon-path" d="M23.977 6.416c-.105 2.338-1.739 5.543-4.894 9.609-3.268 4.247-6.026 6.37-8.29 6.37-1.409 0-2.578-1.294-3.553-3.881L5.322 11.4C4.603 8.816 3.834 7.522 3.01 7.522c-.179 0-.806.378-1.881 1.132L0 7.197a315.065 315.065 0 0 0 3.501-3.128C5.08 2.701 6.266 1.984 7.055 1.91c1.867-.18 3.016 1.1 3.447 3.838.465 2.953.789 4.789.971 5.507.539 2.45 1.131 3.674 1.776 3.674.502 0 1.256-.796 2.265-2.385 1.004-1.589 1.54-2.797 1.612-3.628.144-1.371-.395-2.061-1.614-2.061-.574 0-1.167.121-1.777.391 1.186-3.868 3.434-5.757 6.762-5.637 2.473.06 3.628 1.664 3.493 4.797l-.013.01z" fill={color} />
    </svg>
  );
}
...
export default function Header() {
  ...
  let headerBg: string;
  if (isLightPage) {
    headerBg = "linear-gradient(180deg, #FFFFFF 0%, rgba(255,255,255,0) 100%)";
  } else if (isDarkGradientPage) {
    headerBg = "linear-gradient(to bottom, #010100 0%, #010100 60%, transparent 100%)";
  } else {
    headerBg = "#010100";
  }
  ...
  return (
    <>
      <style>{navCSS}</style>
      <header
        className="fixed left-0 right-0 top-0 z-50 flex h-[96px] items-center justify-center"
        style={{ background: headerBg }}
      >
        <nav className="flex h-[60px] w-full max-w-[1340px] items-center justify-between px-[50px]">
              ...
              link.dropdown ? (
                <div key={link.to} className="dropdown-wrapper">
                  <NavLink
                    to={link.to}
                    className={({ isActive }) =>
                      `nav-link flex items-center gap-2 font-sans text-[20px] leading-6 ${isActive ? "active" : ""}`
                    }
                    style={({ isActive }) => ({
                      color: isActive ? activeColor : textColor,
                      filter: !isLightPage
                        ? "drop-shadow(0px 4px 5.8px rgba(0,0,0,0.4))"
                        : "drop-shadow(0px 4px 5.8px rgba(0,0,0,0.15))",
                    })}
                  >
                    <span>{currentLang === "en" ? link.labelEn : link.labelEs}</span>
                    <svg width="10" height="6" viewBox="0 0 10 6" fill="none" className="ml-1 transition-transform" style={{ transform: "rotate(0deg)" }}>
          ...
          {/* ═══ Right: Social Icons (30px gap) Flags ═══ */}
          <div className="hidden items-center md:flex" style={{ gap: "30px" }}>

            {/* Social Icons — inline SVGs, no background frames */}
            <div className="flex items-center" style={{ gap: "0px" }}>
              {socialLinks.map((social) => (
                <a
                  ...
                  className="social-icon-link flex h-[50px] w-[46px] items-center justify-center"
                >
            ...
            <div className="flex items-center" style={{ gap: "7px" }}>
              <button
                ...
                className={`flex h-[32px] w-[32px] items-center justify-center overflow-hidden rounded-full transition-opacity ${
                  currentLang.startsWith("es") ? "opacity-100" : "opacity-60 hover:opacity-100"
                }`}
                aria-label="Español"
              >
                <img src="/assets/flag-mx.svg" alt="México" className="h-full w-full object-cover" />
              </button>
              <button
                onClick={() => changeLanguage("en")}
                className={`flex h-[32px] w-[32px] items-center justify-center overflow-hidden rounded-full transition-opacity ${
                  currentLang.startsWith("en") ? "opacity-100" : "opacity-60 hover:opacity-100"
                }`}
                aria-label="English"
              >
                <img src="/assets/flag-us.svg" alt="USA" className="h-full w-full object-cover" />
              </button>
            </div>
          </div>
          ...
          >
            {[0, 1, 2].map((i) => (
              <span
                key={i}
                className="h-[3px] w-7 transition-transform"
                style={{
      ...
      {menuOpen && (
        ...
        >
          {/* Top bar — Figma: logo 70x60 at left:21 top:34, X icon at left:365 top:40 */}
          ...
          {/* Nav links — Figma: left:46px, 36px font, #F8F1CD */}
          <div style={{ display: "flex", flexDirection: "column", paddingLeft: "46px", paddingRight: "46px", paddingTop: "60px", gap: "0px" }}>
            ...
            <div>
              ...
              >
                ...
                <svg width="12" height="8" viewBox="0 0 12 8" fill="none" style={{ transform: projectsExpanded ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s ease" }}>
                  <path d="M1 1L6 6L11 1" stroke="#F8F1CD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>

              {/* Subcategories — Figma: 24px font, left:85px relative to menu */}
              {projectsExpanded && (
          ...
          {/* Bottom: social icons + flags — Figma: icons at left:26 top:793, flags at left:302 top:793 */}
          <div style={{
            position: "absolute", bottom: "60px", left: "26px", right: "26px",
            display: "flex", alignItems: "center", justifyContent: "space-between",
          }}>
            {/* Social Icons — 57x62 containers, 37px icons */}
            ...
            {/* Flags — 42x42 inside 58x58 containers */}
            <div style={{ display: "flex", alignItems: "center", gap: "0px" }}>
              <button
                onClick={() => changeLanguage("es")}
                style={{
