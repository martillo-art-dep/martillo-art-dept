...
// scale: multiplicador sobre el max-h base (1 = normal, 1.5 = 50% más grande, etc.)
const LOGOS = [
  ...
  { src: "/assets/logos/21st-century-fox.svg",               alt: "21st Century Fox",   rounded: false, scale: 1.8  }, // SVG con mucho whitespace interno
...
function LogoItem({ src, alt, rounded = false, scale = 1 }: LogoItemProps) {
  const [hasError, setHasError] = useState(false);

  // Base heights por breakpoint
  const mobileH  = Math.round(24 * scale);
  const tabletH  = Math.round(40 * scale);
  const desktopH = Math.round(56 * scale);

  return (
    <div className="flex shrink-0 items-center justify-center w-[72px] md:w-[120px] lg:w-[160px] h-[60px] md:h-[72px] lg:h-[84px]">
      {!hasError ? (
        <img
          src={src}
          alt={alt}
          ...
          style={{
            maxHeight: mobileH,        // se sobreescribe con media query abajo
            maxWidth: "100%",
            borderRadius: rounded ? 8 : undefined,
            // Media queries inline no funcionan — usamos CSS custom properties vía style tag trick:
            // En su lugar, usamos data-attr para las clases responsive
          }}
          // Trick: sobreescribir con clases responsive usando un wrapper invisible
...
export default function LogoMarquee() {
  return (
    <>
      {/* Estilos responsive para alturas de logos con scale individual */}
      <style>{`
        .logo-item img {
        ...
        <div className="flex h-full items-center overflow-hidden">
          <div className="logo-marquee-track flex shrink-0 items-center gap-[24px] px-[12px] md:gap-[48px] md:px-[24px] lg:gap-[80px] lg:px-[40px]">
            {[...LOGOS, ...LOGOS].map((logo, i) => {
              const mobileH  = Math.round(24 * (logo.scale ?? 1));
              const tabletH  = Math.round(40 * (logo.scale ?? 1));
              const desktopH = Math.round(56 * (logo.scale ?? 1));
